...
inference_policy.json -- the training-side implementation, used by
inference_policy_generator.py to validate what it generates.

Deployment boundary: the API MUST NOT import this module at runtime, and
must not import from training/ at all. Task 4 ships a byte-identical copy
at api/policy_schema.py, kept in sync by a test in
training/test_policy_generator.py asserting the two files hash identically.
inference_policy.json is the stage boundary; this file is duplicated across
it, not imported across it. Task 4 must validate its API-local copy against
the same frozen policy cases this module is tested against.
